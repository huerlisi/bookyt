class Mt940Attachment < Attachment
  ...
  def reference
    by_class(MT940::Job).first.reference
  end
  ...
  def start_date
    by_class(MT940::OpeningBalance).first.date
  end

  def end_date
    by_class(MT940::ClosingBalance).first.date
  end

  def account
    BankAccount.where("REPLACE(iban, ' ', '') = ?", account_identifier).first
  end
